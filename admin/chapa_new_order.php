        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            	<?php
$i = 0;
$statement = $pdo->prepare("SELECT * FROM tbl_payment where cust_type='Individual' AND payment_method='Chapa' AND payment_status='Pending' ");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $i++;
...
$statement1 = $pdo->prepare("SELECT * FROM tbl_order WHERE payment_id=? ");
    ...
    ?>
	<!--<td ><img id="myImg"  src="../assets/uploads/maintenance_photos/<?php echo $row['photo_name']; ?>" alt="<?php echo $row['cust_name']; ?>" style="width:100px; max-width:300px;"></td>-->

		<td ><img id="myImg" src="../assets/uploads/prescription_photos/<?php echo $row1['photo']; ?>" alt="<?php  echo $row['customer_name'] ?>" style="width:100px;  max-width:300px;"></td>
...
if ($row['promo_status'] == 'Pending') {
        ?>
                                    <a href="chapa_promo_comp.php?id=<?php echo $row['id']; ?>&pro=<?php echo $row['promo_code'] ?>&discount=<?php echo $row['discount'] ?>" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Make Completed</a>
                                    <?php
}
    ?>
                        </td>
...
if ($row['payment_status'] == 'Pending') {
        ?>
                                    <a href="chapa_payment_status.php?id=<?php echo $row['id']; ?>&task=Completed" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Make Completed</a>
                                    <?php
}
    ?>
                        </td>
	                    ...
	                    <td>
                            <a href="#" class="btn btn-danger btn-xs" data-href="chapa_order_delete.php?id=<?php echo $row['id']; ?>" data-toggle="modal" data-target="#confirm-delete" style="width:100%;">Delete</a>
	                    </td>
	                </tr>
            		<?php
}
