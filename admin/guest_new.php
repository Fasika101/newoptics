        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            	<?php
$i = 0;
$statement = $pdo->prepare("SELECT * FROM tbl_payment where  payment_method='Chapa' AND cust_type='Guest' AND payment_status='Pending' ");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $i++;
...
$statement1 = $pdo->prepare("SELECT * FROM tbl_order WHERE payment_id=? ");
    ...
    foreach ($result1 as $row1) {
        ...
        $me_price = $row1['unit_price'];
    }
    ?>
									<td style="width:130px;"><img src="../assets/uploads/prescription_photos/<?php echo $row1['photo']; ?>" alt="<?php ?>" style="width:100px;"></td>
  <?php
?>
...
$statement2 = $pdo->prepare("SELECT * FROM promo WHERE promo_code=?");
    ...
    ?>
                        ...
                        <td><?php echo '<br>Unit Price :</br> <b>' . $me_price . '</b> ETB';
...
if ($row['promo_status'] == 'Pending') {
        ?>
                                    <a href="guest_promo_comp.php?id=<?php echo $row['id']; ?>&pro=<?php echo $row['promo_code'] ?>&discount=<?php echo $row['discount'] ?>" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Make Completed</a>
                                    <?php
}
    ?>
                        </td>
...
if ($row['payment_status'] == 'Pending') {
        ?>
                                    <a href="guest_payment_status.php?id=<?php echo $row['id']; ?>&task=Completed" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Make Completed</a>
                                    <?php
}
    ?>
                        </td>
	                    ...
	                    <td>
                            <a href="#" class="btn btn-danger btn-xs" data-href="guest_order_delete.php?id=<?php echo $row['id']; ?>" data-toggle="modal" data-target="#confirm-delete" style="width:100%;">Delete</a>
	                    </td>
