...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		$final_name = 'product-featured-'.$_REQUEST['id'].date('mdYHis').'.'.$ext;
		...
		if(isset($_POST['color'])) {
			...
			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_color (color_id,p_id,color_qty) VALUES (?,?,?)");
				$statement->execute(array($value,$_REQUEST['id'],'0'));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));
...
if(isset($_POST['color_qty']) ) {


	$input = $_POST['color_qty'];
    $id =  $_POST['color_ids'];
	$idid = $_POST['idid'];

	$i = 0;
foreach ($_POST['color_ids'] as $key => $value) {
    $i++;
    $arr_c_id[$i] = $value;
}
$i = 0;
...
$i = 0;
foreach ($_POST['idid'] as $key => $value) {
    $i++;
    $arr_p_id[$i] = $value;
}
$i = 0;
...
foreach ($_POST['color_qty'] as $key => $value) {
    $i++;
    $arr_c_qty[$i] = $value;
}
// foreach($_POST['color_qty'] as $value) {
// 	$statement = $pdo->prepare("UPDATE tbl_product_color SET color_qty=? WHERE color_id=? AND p_id=?");
//     $statement->execute(array($arr_c_qty[$i], $arr_c_id[$i], $arr_p_id[$i] ));
// }
	for ($i = 1; $i <= count($arr_p_id); $i++) {
    // $statement = $pdo->prepare("UPDATE tbl_product_color ( product_id, p_scratchresistant_check) VALUES (?,?)");
...
$statement7->execute(array($_REQUEST['id']));
...
$statement7->execute(array($sum, $_REQUEST['id']));
...
foreach ($result as $row) {
	...
	$p_short_description = $row['p_feature'];
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Brand Type<span>*</span></label>
							<div class="col-sm-4">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Color 	qty</label>
							<div class="col-sm-4">
									...
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										$statement23 = $pdo->prepare("SELECT * FROM tbl_color WHERE color_id = ? ");
										$statement23->execute(array($row['color_id']));
										$result23 = $statement23->fetchAll(PDO::FETCH_ASSOC);	
										foreach ($result23 as $row2) {
									?>
										<div class="form-group mb-3">
											
                                                    <label for=""><?php echo $row2['color_name']; ?></label>
                                                    <input type="hidden"  id="color_ids" name="color_ids[]" value="<?php echo $row['color_id']; ?>" class="form-control">
                                                    <input  id="idid" name="idid[]" value="<?php echo $row['p_id']; ?>" class="form-control">
                                                    <input id="color_qty" name="color_qty[]" value="<?php echo $row['color_qty']; ?>" class="form-control">
                                         </div>
										<?php
										}
									}
						...
						<!-- <div class="form-group">
							<label for="" class="col-sm-3 control-label">Product Size Description</label>
							<div class="col-sm-8">
						...
						<!-- <div class="form-group">
							<label for="" class="col-sm-3 control-label">Product Size Description</label>
							<div class="col-sm-8">
								<textarea name="p_short_description" class="form-control" cols="30" rows="10" id="editor3"><?php echo $p_short_description; ?></textarea>
							</div>
						</div> -->
						<!-- <div class="form-group">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Show on Exclusive products page?</label>
							<div class="col-sm-8">
