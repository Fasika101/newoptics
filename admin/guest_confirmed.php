...
<section class="content-header">
	<div class="content-header-left">
		<h1>Guest | Chapa Payment Confirmed Orders</h1>
	</div>
</section>
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            	<?php
$i = 0;
$statement = $pdo->prepare("SELECT * FROM tbl_payment where cust_type='Guest' AND payment_method='Chapa' AND payment_status='Completed' AND shipping_status='Pending'");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $i++;
...
if ($row['payment_status'] == 'Completed') {
        if ($row['shipping_status'] == 'Pending') {
            ?>
                                    <a href="guest_shipping_status.php?id=<?php echo $row['id']; ?>&task=Completed" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Make Completed</a>
                                    <?php
}
    }
    ?>
